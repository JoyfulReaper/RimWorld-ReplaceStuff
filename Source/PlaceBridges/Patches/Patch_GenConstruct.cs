...
class CanPlaceBlueprint
{
    ...
    {
        var posInfo = method.GetMethodBody().LocalVariables.First(lv => lv.LocalType == typeof(IntVec3));
        var getAffordances = AccessTools.Method(typeof(GridsUtility), nameof(GridsUtility.GetAffordances));
        var listContains = AccessTools.Method(typeof(List<TerrainAffordanceDef>), nameof(List<TerrainAffordanceDef>.Contains));
        ...
        for (int i = 0; i < instList.Count(); i++)
        {
            ...
            if (inst.Calls(getAffordances) && firstOnly)
            {
                ...
                yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(CanPlaceBlueprint), nameof(EvaluateAffordanceOrBridge)));
            }
            else
                yield return inst;
        }
    ...
    public static bool EvaluateAffordanceOrBridge(List<TerrainAffordanceDef> affordances, TerrainAffordanceDef neededDef, BuildableDef def, IntVec3 pos, Map map)
    {
        //Code Used to be:
...
class InterceptBlueprintPlaceBridgeFrame
{
    ...
    public static void Prefix(BuildableDef sourceDef, IntVec3 center, Map map, Rot4 rotation, Faction faction, ThingDef stuff)
    {
        ...
        foreach (IntVec3 pos in GenAdj.CellsOccupiedBy(center, rotation, sourceDef.Size))
            BridgeUtility.PlaceBridgeIfNeeded(sourceDef, pos, map, rotation, faction, stuff);
    }
}